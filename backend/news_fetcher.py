...
def _fetch_fmp_news(ticker: str, api_key: str) -> List[NewsItem]:
    """Fetch recent news from FMP stock_news endpoint."""
    items = []

    # Try multiple endpoint variants — FMP has moved this between API versions
    endpoints = [
        f"https://financialmodelingprep.com/api/v3/stock_news?tickers={ticker}&limit={MAX_NEWS}&apikey={api_key}",
        f"https://financialmodelingprep.com/stable/stock-news?symbol={ticker}&limit={MAX_NEWS}&apikey={api_key}",
        f"https://financialmodelingprep.com/api/v3/stock_news?symbol={ticker}&limit={MAX_NEWS}&apikey={api_key}",
    ]
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if result and isinstance(result, list) and len(result) > 0:
                    data = result
                    print(f"[News] FMP: {len(data)} articles from {url.split('?')[0]}")
                    break
        except Exception:
            continue

    if not data:
        print(f"[News] FMP: all news endpoints failed for {ticker}")
        return items
