...
def fetch_ddm(ticker: str, sector_info: Optional[SectorInfo] = None) -> DDMResult:
    ...
    # Dividend history — FMP historical-price-eod-dividend-list
    try:
        dividends_raw = _fetch(
    ...
    for d in dividends_raw:
        date_str = d.get("date") or d.get("paymentDate") or ""
        if not date_str or len(date_str) < 4:
            continue
        try:
            year = int(date_str[:4])
            amount = float(d.get("dividend") or d.get("adjDividend") or 0)
            if amount > 0:
