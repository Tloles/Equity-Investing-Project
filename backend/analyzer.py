...
_ANALYSIS_TOOL = {
    ...
    "input_schema": {
        ...
        "properties": {
            "overall_rating": {
                "type": "string",
                "enum": ["Strong Buy", "Bullish", "Neutral", "Bearish", "Strong Sell"],
                "description": (
                    "Your overall investment rating for this stock based on the "
                    "weight of evidence across fundamentals, valuation, risks, "
            ...
            "thesis_statement": {
                "type": "string",
                "description": (
                    "A single sentence (max 25 words) capturing the core investment "
                    "thesis. Example: 'Durable compounder with pricing power, but "
                    "flat volumes and FX headwinds cap near-term upside.'"
                ),
            },
            "key_metrics": {
                "type": "array",
                "items": {
                    ...
                    "properties": {
                        "label": {
                            "type": "string",
                            "description": "Short metric name, e.g. 'Organic Revenue Growth', 'Operating Margin', 'FCF Yield', 'Dividend Yield'",
                        },
                        "value": {
                            "type": "string",
                            "description": "The metric value as a formatted string, e.g. '5%', '$11.4B', '31.2%'",
                        },
                        "trend": {
                            "type": "string",
                            "enum": ["up", "flat", "down"],
                            "description": "Direction of this metric vs. prior year.",
                        },
                    },
                    "required": ["label", "value", "trend"],
                },
                "description": (
                    "Exactly 4-5 key financial metrics that define the investment "
                    "story. Pick the most decision-relevant metrics for this specific "
                    "company and sector. Always include at least one growth, one "
                    "profitability, and one cash flow metric."
                ),
            },
            "bull_case": {
                "type": "array",
                "items": {
                    ...
                    "properties": {
                        "headline": {
                            "type": "string",
                            "description": "One-line summary of this bull argument (max 10 words).",
                        },
                        "detail": {
                            "type": "string",
            ...
            "bear_case": {
                ...
                "items": {
                    ...
                    "properties": {
                        "headline": {
                            "type": "string",
                            "description": "One-line summary of this bear argument (max 10 words).",
                        },
                        "detail": {
                            "type": "string",
                            "description": "Full evidence-backed explanation (2-4 sentences).",
...
class AnalysisResult:
    ...
    key_metrics: list  # list of dicts with label, value, trend
    bull_case: list    # list of dicts with headline, detail
    bear_case: list    # list of dicts with headline, detail
...
1. An overall rating (Strong Buy / Bullish / Neutral / Bearish / Strong Sell)
2. A one-sentence thesis statement (max 25 words) — the single most important takeaway
3. 4-5 key financial metrics with values and y/y trend direction — pick the metrics
   most relevant to THIS company's investment story (always include growth, profitability,
   and cash flow; add sector-specific metrics like dividend yield, same-store sales, etc.)
4. Three specific bull case arguments, each with a short headline (max 10 words) and
   a detailed evidence-backed explanation (2-4 sentences)
5. Three specific bear case arguments, same format as bull case
6. Key risks that management appears to be downplaying or omitting
7. Recent catalysts — 2-4 near-term events or developments from the news/social data
   that could materially impact the stock (leave empty if no news data was provided)
8. A 1-2 sentence sentiment summary describing the current market narrative and mood
...
def analyze(
    ...
    print(f"[analysis] Claude returned keys: {list(data.keys())}")
    print(f"[analysis] rating={data.get('overall_rating')}  thesis={data.get('thesis_statement','')[:80]}")

    # Normalize bull/bear — handle both structured {headline, detail} and legacy strings
    def _normalize_cases(items):
        ...
        for item in items:
            if isinstance(item, dict):
                ...
                # Legacy: split on first colon or use full string
                if ":" in item:
                    headline, detail = item.split(":", 1)
                    result.append({"headline": headline.strip(), "detail": detail.strip()})
                else:
                    result.append({"headline": item[:60], "detail": item})
        return result

    # Normalize key_metrics
    raw_metrics = data.get("key_metrics") or []
    key_metrics = []
    ...
    return AnalysisResult(
        ticker=ticker.upper(),
        overall_rating=data.get("overall_rating") or "Neutral",
        thesis_statement=data.get("thesis_statement") or "",
        key_metrics=key_metrics,
        bull_case=_normalize_cases(data.get("bull_case") or []),
        bear_case=_normalize_cases(data.get("bear_case") or []),
