...
class AnalysisResponse(BaseModel):
    ...
    # Finviz supplementary data (best-effort)
    finviz: Optional[FinvizModel] = None
...
async def analyze_ticker(ticker: str) -> AnalysisResponse:
    ...
    tenk_result, transcript_result, dcf_result, ddm_result, financials_result, finviz_result, news_result = results

    # Finviz is optional — degrade gracefully
    if isinstance(finviz_result, Exception):
        print(f"[main] Finviz failed — {type(finviz_result).__name__}: {finviz_result}")
        finviz_data = None
    else:
