...
function renderDividendHistory(history) {
  ...
  for (const h of history) {
    dpsRow += `<td>${ddmFmtDollar(h.annual_dps)}</td>`;
    growthRow += `<td>${ddmDash(h.dps_growth, ddmFmtPct)}</td>`;
    payoutRow += `<td>${ddmDash(h.payout_ratio, ddmFmtPct)}</td>`;
  }
...
function renderDDM(data) {
  ...
  renderDividendHistory(data.ddm_model.history);
